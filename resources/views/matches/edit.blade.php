    ...
    <div class="container mx-auto p-4">
        ...
        <form action="{{ route('matches.update', $match->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="team1_id" id="team1_id" value="{{ $match->team1_id }}" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md" />
            </div>

            <div class="mb-4">
                <label for="team2_id" class="block text-sm font-medium text-gray-700">Team 2 ID</label>
                <input type="number" name="team2_id" id="team2_id" value="{{ $match->team2_id }}" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md" />
            </div>

            <div class="mb-4">
                <label for="match_date" class="block text-sm font-medium text-gray-700">Match Date</label>
                <input type="datetime-local" name="match_date" id="match_date" value="{{ $match->match_date }}" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md" />
            </div>

            <div class="mb-4">
                <label for="venue" class="block text-sm font-medium text-gray-700">Venue</label>
                <input type="text" name="venue" id="venue" value="{{ $match->venue }}" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md" />
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="singles_player1_team1" name="singles_player1_team1" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                    @foreach ($players as $player)
                        <option value="{{ $player->id }}" {{ $player->id == $match->singles_player1_team1 ? 'selected' : '' }}>{{ $player->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="singles_player2_team1" name="singles_player2_team1" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                    @foreach ($players as $player)
                        <option value="{{ $player->id }}" {{ $player->id == $match->singles_player2_team1 ? 'selected' : '' }}>{{ $player->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="singles_player1_team2" name="singles_player1_team2" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                    @foreach ($players as $player)
                        <option value="{{ $player->id }}" {{ $player->id == $match->singles_player1_team2 ? 'selected' : '' }}>{{ $player->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="singles_player2_team2" name="singles_player2_team2" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                    @foreach ($players as $player)
                        <option value="{{ $player->id }}" {{ $player->id == $match->singles_player2_team2 ? 'selected' : '' }}>{{ $player->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="doubles_player1_team1" name="doubles_player1_team1" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                    @foreach ($players as $player)
                        <option value="{{ $player->id }}" {{ $player->id == $match->doubles_player1_team1 ? 'selected' : '' }}>{{ $player->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="doubles_player2_team1" name="doubles_player2_team1" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                    @foreach ($players as $player)
                        <option value="{{ $player->id }}" {{ $player->id == $match->doubles_player2_team1 ? 'selected' : '' }}>{{ $player->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="doubles_player1_team2" name="doubles_player1_team2" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                    @foreach ($players as $player)
                        <option value="{{ $player->id }}" {{ $player->id == $match->doubles_player1_team2 ? 'selected' : '' }}>{{ $player->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="doubles_player2_team2" name="doubles_player2_team2" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md">
                    @foreach ($players as $player)
                        <option value="{{ $player->id }}" {{ $player->id == $match->doubles_player2_team2 ? 'selected' : '' }}>{{ $player->name }}</option>
                    @endforeach
                </select>
            </div>

            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">Update Match</button>
        </form>
    </div>
</body>
</html>
